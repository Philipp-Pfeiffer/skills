...
![Everything to the left of the dotted line runs automatically. Everything to the right runs on your time, which is where production debugging actually lives.](images/image_001.jpg)
...
![Most platforms end at the dashboard and hand the rest to you. The right side is the loop Opik runs on its own.](images/image_006.jpg)
*Most platforms end at the dashboard and hand the rest to you. The right side is the loop Opik runs on its own.*

# [Opik: AI Observability & Evals For the Agentic Era](https://github.com/comet-ml/opik)
Opik is an [**open-source**](https://github.com/comet-ml/opik) logging, debugging, and optimization platform for AI agents and LLM applications. Opik is built around the premise that this loop should be automated, not staffed.
...
![The four layers are not separate features. They feed each other in a single loop that closes on its own.](images/image_005.jpg)
...
![The four layers are not separate features. They feed each other in a single loop that closes on its own.](images/image_009.jpg)
*The four layers are not separate features. They feed each other in a single loop that closes on its own.*
...
![Ollie working through a fix in the side panel, reading and editing files only after you approve each step.](images/image_007.jpg)
...
![The full path from a bad trace to a locked regression test, with your approval as the one manual step.](images/image_002.jpg)
...
![A regression suite built from real failures, with each assertion written as a plain-English check.](images/image_003.jpg)
...
![The same loop drawn end to end, from instrumenting your agent to the next failure entering at the top.](images/image_004.jpg)
...
[**Check out Comet ML and Opik →**](https://github.com/comet-ml/opik) 
( don't forget to star ![](https://abs.twimg.com/emoji/v2/svg/1f31f.svg))
...
— [Original post on X](https://x.com/akshay_pachaar/status/2064051835636498924)